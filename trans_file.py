...
def find_valid_urls():
    ...
    try:
        tld = "|".join(
            get("https://data.iana.org/TLD/tlds-alpha-by-domain.txt").text.split("\n")[1:])[:-1]
        regex += rf"({tld})"
...
def make_block_list(new_file_name: str):
    ...
    for file in filenames:
        ...
        if "word/document.xml" not in document.namelist():
            raise Exception(
                "didn't find needed attr in file xml stuture please add this feature to fix"
            )
        parsed_xml = parseString(
            document.read("word/document.xml", pwd=None)
        ).toprettyxml(indent=" ")
        ...
        for url_tup in url_list_raw:
            if all(
                [
                    "schemas.microsoft.co" not in url_tup[0],
                    "schemas.openxml" not in url_tup[0],
                ]
            ):
...
def term_trans():
    ...
    mandatory_args.add_argument(
        ...
        required=True,
        type=str,
        help="a unique output file name that will be appended to the file(s)",
    )
